...
<root xmlns="http://www.vips.ecs.soton.ac.uk/nip/7.34.1">
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="476" lpane_open="true" rpane_position="100" rpane_open="false" local_defs="// private definitions for this workspace&#10;&#10;get_option options defaults f&#10;    = error (_ &quot;unknown parameter &quot; ++ f), hits == []&#10;    = hits?0&#10;{&#10;    hits = [v :: [n, v] &lt;- options ++ defaults; n == f];&#10;}&#10;&#10;mkim options x y b&#10;    = Image (image_new x y b&#10;        (opt $format) (opt $coding) (opt $type)&#10;        (opt $pixel)&#10;        (opt $xoffset) (opt $yoffset))&#10;{&#10;    opt = get_option options [&#10;        $format =&gt; Image_format.UCHAR,&#10;        $coding =&gt; Image_coding.NOCODING,&#10;        $type =&gt; Image_type.sRGB,&#10;        $pixel =&gt; 0,&#10;        $xoffset =&gt; 0,&#10;        $yoffset =&gt; 0&#10;    ];&#10;}&#10;&#10;is_Pair = is_instanceof &quot;Pair&quot;;&#10;&#10;Pair l a b = class&#10;    _Object {&#10;&#9;_vislevel = 2;&#10;&#10;&#9;label = l;&#10;&#9;targets = a;&#10;&#9;object = b;&#10;&#9;&#10;    oo_binary_table op x = [&#10;        [this.Pair label (op.fn targets x.targets) (op.fn object x.object),&#10;            is_Pair x],&#10;        [this.Pair label (op.fn a x) (op.fn b x),&#10;            true]&#10;    ];&#10;&#9;oo_unary_table op = [&#10;&#9;&#9;[this.Pair label (op.fn a) (op.fn b),&#10;&#9;&#9;&#9;true]&#10;&#9;];&#10;}&#10;&#10;Pair_new l x y = Pair l (to_image x.value) (to_image y.value);&#10;&#10;Pair_flatfield _i label = class {&#10;    _vislevel = 3;&#10;&#10;    image &#10;&#9;&#9;= mkim [] 10 10 3, NULL == _i&#10;&#9;&#9;= _i;&#10;    use_flatfield = Toggle (&quot;Flatfield &quot; ++ label ++ &quot; image&quot;) false;&#10;    flatfield = mkim [$pixel =&gt; 200] image.width image.height 3;&#10;}&#10;&#10;Pair_load label = class {&#10;&#9;_vislevel = 2;&#10;&#9;targets = Pair_flatfield NULL (label ++ &quot; with calibration targets&quot;);&#10;&#9;object = Pair_flatfield (targets.image) (label ++ &quot; of object&quot;);&#10;}&#10;&#10;draw_samples im across down measure&#10;    = mark&#10;{&#10;    patch_width = im.width / across;&#10;    sample_width = patch_width * (measure / 100);&#10;    left_margin = (patch_width - sample_width) / 2;&#10;    patch_height = im.height / down;&#10;    sample_height = patch_height * (measure / 100);&#10;    top_margin = (patch_height - sample_height) / 2;&#10;&#10;    cods = [[x * patch_width + left_margin, y * patch_height + top_margin] ::  &#10;        y &lt;- [0 .. down - 1]; x &lt;- [0 .. across - 1]];&#10;    x = map (extract 0) cods;&#10;    y = map (extract 1) cods;&#10;&#10;    outer = mkim [$pixel =&gt; 255] sample_width sample_height 1;&#10;    inner = mkim [] (sample_width - 4) (sample_height - 4) 1;&#10;    patch = insert 2 2 inner outer;&#10;&#10;    bg = mkim [] im.width im.height 1;&#10;&#10;    mask = Image (im_insertset bg.value patch.value x y);&#10;&#10;    im' = colour_transform_to Image_type.sRGB im;&#10;&#10;    mark = if mask then Vector [0, 255, 0] else im';&#10;}&#10;&#10;measure_samples im across down measure&#10;    = measures&#10;{&#10;    patch_width = im.width / across;&#10;    sample_width = patch_width * (measure / 100);&#10;    left_margin = (patch_width - sample_width) / 2;&#10;    patch_height = im.height / down;&#10;    sample_height = patch_height * (measure / 100);&#10;    top_margin = (patch_height - sample_height) / 2;&#10;                &#10;    cods = [[x * patch_width + left_margin, y * patch_height + top_margin] ::&#10;        y &lt;- [0 .. down - 1]; x &lt;- [0 .. across - 1]];&#10;        &#10;    patches = map (\p extract_area p?0 p?1 sample_width sample_height im) cods;&#10;    measures = Matrix (map (map mean) (map bandsplit patches));&#10;}&#10;&#10;Colour_chart_to_matrix_item = class&#10;    Menuaction (_ &quot;_Measure Colour Chart&quot;)&#10;        (_ &quot;measure average pixel values for a colour chart image&quot;) {&#10;    action x = class&#10;        _result {&#10;        _vislevel = 3;&#10;&#10;        pacross = Expression (_ &quot;Patches across chart&quot;) 6;&#10;        pdown = Expression (_ &quot;Patches down chart&quot;) 4;&#10;        measure = Scale (_ &quot;Average centre (%)&quot;) 1 100 50;&#10;&#10;        // get a representative image from an arg&#10;        get_image x&#10;            = get_image x.value?0, is_Group x&#10;            = x;&#10;        _im = get_image x; &#10;&#10;        // and display it with a visualisation of the sample patches&#10;        sample = draw_samples _im&#10;            (to_real pacross) (to_real pdown) (to_real measure);&#10;&#10;        _result&#10;            = map_unary chart x&#10;        {&#10;            chart in&#10;                = measure_samples in&#10;                    (to_real pacross) (to_real pdown) (to_real measure);&#10;        }&#10;    }&#10;}&#10;&#10;" name="input" caption="all the source images" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
    ...
    <Column x="681" y="794" open="true" selected="false" sform="false" next="1" name="F" caption="UV reflectance">
      <Subcolumn vislevel="3">
    ...
    <Column x="1367" y="0" open="true" selected="false" sform="false" next="3" name="G" caption="UV-induced visible luminescence">
      <Subcolumn vislevel="3">
    ...
    <Column x="1367" y="794" open="true" selected="true" sform="false" next="1" name="H" caption="visible-induced IR luminescence">
      <Subcolumn vislevel="3">
    ...
    <Column x="0" y="620" open="true" selected="false" sform="false" next="5" name="I" caption="save output images as 16-bits?">
      <Subcolumn vislevel="3">
  ...
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="502" lpane_open="true" rpane_position="100" rpane_open="false" local_defs="// private definitions for this workspace&#10;&#10;straighten arrow&#10;&#9;= rotate Interpolate_bilinear angle'' arrow.image&#10;{&#10;&#9;x = arrow.width;&#10;&#9;y = arrow.height;&#10;&#10;&#9;angle = im (polar (x, y));&#10;&#9;&#9;&#10;&#9;angle'&#10;&#9;&#9;= angle - 360, angle &gt; 315&#10;&#9;&#9;= angle - 180, angle &gt; 135&#10;&#9;&#9;= angle;&#10;&#9;&#9;&#10;&#9;angle''&#10;&#9;&#9;= -angle', angle' &gt;= (-45) &amp;&amp; angle' &lt; 45&#10;&#9;&#9;= 90 - angle';&#10;}&#10;&#10;rotate_widget = Image_transform_item.Rotate_item.Fixed_item.rotate_widget;&#10;&#10;Markup_macbeth pair = class {&#10;&#9;_vislevel = 2;&#10;&#10;&#9;straighten_prompt = &quot;Position line along edge of Macbeth in &quot; ++ pair.label;&#10;&#9;straighten_image = copy pair.targets;&#10;&#9;line &#10;&#9;&#9;= Arrow straighten_image x y w h&#10;&#9;{&#10;&#9;&#9;x = pair.targets.width / 4;&#10;&#9;&#9;y = pair.targets.height / 2;&#10;&#9;&#9;w = pair.targets.width / 2;&#10;&#9;&#9;h = 0;&#10;&#9;}&#10;&#10;&#9;enclose_prompt = &quot;Enclose the Macbeth with box&quot;;&#10;&#9;box_image = straighten line;&#10;&#9;box &#10;&#9;&#9;= Region box_image x y w h &#10;&#9;{&#10;&#9;&#9;x = box_image.width / 4;&#10;&#9;&#9;y = box_image.height / 4;&#10;&#9;&#9;w = box_image.width / 2;&#10;&#9;&#9;h = box_image.height / 2;&#10;&#9;}&#10;&#10;&#9;rotate_prompt = &quot;Rotate the Macbeth to get white at the bottom-left&quot;;&#10;&#9;rotate &#10;&#9;&#9;= rotate_widget 0 box;&#10;}&#10;&#10;Markup_reflectance default pair =&#9;class {&#10;&#9;_vislevel = 2;&#10;&#10;&#9;straighten_prompt = &quot;Position line along edge of reflectance standard in &quot; ++ pair.label;&#10;&#9;straighten_image = copy pair.targets;&#10;&#9;line &#10;&#9;&#9;= Arrow straighten_image x y w h, NULL == default&#10;&#9;&#9;= Arrow straighten_image default.line.left default.line.top &#10;&#9;&#9;&#9;default.line.width default.line.height&#10;&#9;{&#10;&#9;&#9;x = pair.targets.width / 4;&#10;&#9;&#9;y = pair.targets.height / 2;&#10;&#9;&#9;w = pair.targets.width / 2;&#10;&#9;&#9;h = 0;&#10;&#9;}&#10;&#10;&#9;enclose_prompt = &quot;Enclose the reflectance standard with box&quot;;&#10;&#9;box_image = straighten line;&#10;&#9;box &#10;&#9;&#9;= Region box_image x y w h, NULL == default&#10;&#9;&#9;= Region box_image default.box.left default.box.top &#10;&#9;&#9;&#9;default.box.width default.box.height &#10;&#9;{&#10;&#9;&#9;x = box_image.width / 4;&#10;&#9;&#9;y = box_image.height / 4;&#10;&#9;&#9;w = box_image.width / 2;&#10;&#9;&#9;h = box_image.height / 2;&#10;&#9;}&#10;&#10;&#9;rotate_prompt = &quot;Rotate the reflectance standard to get white at the bottom-left&quot;;&#10;&#9;rotate &#10;&#9;&#9;= rotate_widget 0 box, NULL == default&#10;&#9;&#9;= rotate_widget default.rotate.angle.value box;&#10;&#10;&#9;pacross = Expression (_ &quot;Patches across chart&quot;)&#10;&#9;&#9;&#9;(if NULL == default then 1 else to_real default.pacross); &#10;&#9;centre = Scale (_ &quot;Take centre (%)&quot;) 1 100 &#10;&#9;&#9;&#9;(if NULL == default then 50 else to_real default.centre);&#10;&#10;&#9;sample = input.draw_samples rotate (to_real pacross) 1 (to_real centre);&#10;&#9;matrix = input.measure_samples rotate (to_real pacross) 1 (to_real centre);&#10;}" name="markup" caption="mark features on images" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
          ...
          <Rhs vislevel="2" flags="6">
            <Subcolumn vislevel="1">
              ...
              <Row name="straighten_image">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="831" window_y="93" window_width="752" window_height="749" image_left="2625" image_top="891" image_mag="1" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="line">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="2352" top="774" width="666" height="30">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="box_image">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="833" window_y="92" window_width="750" window_height="750" image_left="2822" image_top="1256" image_mag="-2" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="box">
                <Rhs vislevel="1" flags="1">
                  <iRegion image_left="0" image_top="0" image_mag="0" show_status="false" show_paintbox="false" show_convert="false" show_rulers="false" scale="0" offset="0" falsecolour="false" type="true" left="2388" top="858" width="658" height="452">
                    <iRegiongroup/>
                  </iRegion>
                  <Subcolumn vislevel="0"/>
                  <iText/>
    ...
    <Column x="1388" y="0" open="true" selected="false" sform="false" next="5" name="B" caption="mark position of reflectance standards in visible-light reflectance image">
      <Subcolumn vislevel="3">
        <Row popup="false" name="B1">
          <Rhs vislevel="2" flags="6">
            <Subcolumn vislevel="1">
              ...
              <Row name="straighten_prompt">
                <Rhs vislevel="1" flags="4">
                  <iText formula="&quot;Put line along edge of reflectance targets&quot;"/>
                </Rhs>
              </Row>
              <Row name="straighten_image">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="833" window_y="92" window_width="750" window_height="750" image_left="775" image_top="1161" image_mag="1" show_status="true" show_paintbox="false" show_convert="true" show_rulers="false" scale="1.1501636861417606" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="line">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="547" top="1073" width="1215" height="57">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="box_image">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="833" window_y="92" window_width="750" window_height="750" image_left="1082" image_top="1104" image_mag="-2" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="box">
                <Rhs vislevel="1" flags="1">
                  <iRegion image_left="0" image_top="0" image_mag="0" show_status="false" show_paintbox="false" show_convert="false" show_rulers="false" scale="0" offset="0" falsecolour="false" type="true" left="600" top="940" width="1232" height="300">
                    <iRegiongroup/>
                  </iRegion>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="sample">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="797" window_y="119" window_width="750" window_height="339" image_left="864" image_top="132" image_mag="1" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
    ...
    <Column x="778" y="0" open="true" selected="true" sform="false" next="4" name="C">
      <Subcolumn vislevel="3">
        <Row popup="false" name="C1">
          <Rhs vislevel="3" flags="7">
            <iImage image_left="0" image_top="0" image_mag="0" show_status="false" show_paintbox="false" show_convert="false" show_rulers="false" scale="0" offset="0" falsecolour="false" type="true"/>
            <Subcolumn vislevel="1"/>
            <iText formula="A21.rotate"/>
          </Rhs>
        </Row>
        <Row popup="false" name="C3">
          <Rhs vislevel="3" flags="7">
            ...
            <Subcolumn vislevel="1">
              ...
              <Row name="measure">
                <Rhs vislevel="1" flags="1">
                  <Slider caption="Average centre (%)" from="1" to="100" value="48.872602739726034"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
  ...
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="0" lpane_open="false" rpane_position="400" rpane_open="false" local_defs="// private definitions for this workspace&#10;&#10;apply = Tasks_capture_item.Apply_calib_item.action;&#10;&#10;export x = icc_export input.depth &quot;$VIPSHOME/share/$PACKAGE/data/sRGB.icm&quot; Render_intent.RELATIVE x; &#10;&#10;apply_xyz c x = colour_transform_to Image_type.XYZ (apply c x);&#10;&#10;Apply_calib c p = class &#10;&#9;input.Pair label targets object {&#10;&#9;_vislevel = 2;&#10;&#10;&#9;label = p.label;&#10;&#9;targets_label = p.targets_label ++ &quot;, colour-calibrated&quot;;&#10;&#9;targets = apply_xyz c p.targets;&#10;&#9;object_label = p.object_label ++ &quot;, colour-calibrated&quot;;&#10;&#9;object = apply_xyz c p.object;&#10;}&#10;&#10;&#9;" name="viscalib" caption="generate visible-light calibration from Macbeth" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
    ...
    <Column x="865" y="0" open="true" selected="true" sform="false" next="36" name="C" caption="get reflectance standards from calibrated visible">
      <Subcolumn vislevel="3">
  ...
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="0" lpane_open="false" rpane_position="100" rpane_open="false" local_defs="// private definitions for this workspace&#10;&#10;Match a b = class &#10;&#9;_result {&#10;&#9;_vislevel = 3;&#10;&#10;&#9;reference = copy a;&#10;&#9;adjust = copy b;&#10;&#10;&#9;ap1 = Mark_relative reference 0.25 0.25;&#10;&#9;ap2 = Mark_relative reference 0.75 0.75;&#10;&#9;bp1 = Mark_relative adjust 0.25 0.25;&#10;&#9;bp2 = Mark_relative adjust 0.75 0.75;&#10;&#10;&#9;test_alignment = _result?1 ++ reference?1 ++ 0;&#10;&#10;&#9;_result&#10;&#9;&#9;= Image (im_match_linear reference.value adjust.value&#10;&#9;&#9;&#9;ap1.left ap1.top bp1.left bp1.top&#10;&#9;&#9;&#9;ap2.left ap2.top bp2.left bp2.top);&#10;}   &#10;&#10;Match_default default a b = class &#10;&#9;_result {&#10;&#9;_vislevel = 3;&#10;&#10;&#9;reference = copy a;&#10;&#9;adjust = copy b;&#10;&#10;&#9;ap1 = Mark reference default.ap1.left default.ap1.top;&#10;&#9;ap2 = Mark reference default.ap2.left default.ap2.top;&#10;&#9;bp1 = Mark adjust default.bp1.left default.bp1.top;&#10;&#9;bp2 = Mark adjust default.bp2.left default.bp2.top;&#10;&#10;&#9;test_alignment = _result?1 ++ reference?1 ++ 0;&#10;&#10;&#9;_result&#10;&#9;&#9;= Image (im_match_linear reference.value adjust.value&#10;&#9;&#9;&#9;ap1.left ap1.top bp1.left bp1.top&#10;&#9;&#9;&#9;ap2.left ap2.top bp2.left bp2.top);&#10;}&#10;&#10;" name="align" caption="align all images" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
        ...
        <Row popup="false" name="D26">
          <Rhs vislevel="3" flags="7">
            ...
            <Subcolumn vislevel="1">
              ...
              <Row name="reference">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="443" window_y="315" window_width="750" window_height="750" image_left="2980" image_top="1237" image_mag="8" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="adjust">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="856" window_y="124" window_width="750" window_height="750" image_left="2960" image_top="1264" image_mag="8" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="ap1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="2982" top="1236" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="ap2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="1848" top="732" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="2951" top="1268" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="1829" top="768" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="test_alignment">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="1" window_y="29" window_width="750" window_height="750" image_left="2944" image_top="2696" image_mag="-8" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
        ...
        <Row popup="false" name="D1">
          <Rhs vislevel="3" flags="7">
            ...
            <Subcolumn vislevel="1">
              ...
              <Row name="ap1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="177" top="923" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="ap2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="3367" top="2720" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="196" top="928" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="3351" top="2705" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
    ...
    <Column x="237" y="0" open="true" selected="false" sform="false" next="10" name="E" caption="match UV to vis">
      <Subcolumn vislevel="3">
        ...
        <Row popup="false" name="E8">
          <Rhs vislevel="3" flags="7">
            ...
            <Subcolumn vislevel="1">
              ...
              <Row name="reference">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="28" window_y="389" window_width="750" window_height="750" image_left="2484" image_top="1572" image_mag="-4" show_status="true" show_paintbox="false" show_convert="true" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="adjust">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="802" window_y="397" window_width="750" window_height="750" image_left="2540" image_top="1128" image_mag="-2" show_status="true" show_paintbox="false" show_convert="true" show_rulers="false" scale="35.30607867027215" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              ...
              <Row name="bp1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="3024" top="1266" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="1906" top="760" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="test_alignment">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="1168" window_y="0" window_width="750" window_height="750" image_left="2208" image_top="1914" image_mag="-6" show_status="true" show_paintbox="false" show_convert="true" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
        ...
        <Row popup="false" name="E9">
          <Rhs vislevel="3" flags="7">
            ...
            <Subcolumn vislevel="1">
              ...
              <Row name="reference">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="1079" window_y="375" window_width="750" window_height="750" image_left="2208" image_top="2022" image_mag="-6" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="adjust">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="1" window_y="29" window_width="750" window_height="750" image_left="2208" image_top="1914" image_mag="-6" show_status="true" show_paintbox="false" show_convert="true" show_rulers="false" scale="18.617011578086434" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              ...
              <Row name="bp1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="196" top="930" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="3357" top="2713" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
    ...
    <Column x="464" y="0" open="true" selected="false" sform="false" next="1" name="A" caption="match UV-induced visible luminescence to vis">
      <Subcolumn vislevel="3">
        <Row popup="false" name="A1">
          <Rhs vislevel="1" flags="4">
            <Subcolumn vislevel="0"/>
        ...
        <Row popup="false" name="A2">
          <Rhs vislevel="3" flags="7">
            ...
            <Subcolumn vislevel="1">
              ...
              <Row name="reference">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="0" window_y="70" window_width="750" window_height="750" image_left="2136" image_top="930" image_mag="1" show_status="true" show_paintbox="false" show_convert="true" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="adjust">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="1103" window_y="137" window_width="750" window_height="750" image_left="2242" image_top="1148" image_mag="1" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="ap1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="2275" top="1216" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="ap2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="1847" top="732" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="2442" top="1414" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="2062" top="891" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="test_alignment">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="620" window_y="602" window_width="750" window_height="750" image_left="1434" image_top="978" image_mag="-2" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
        ...
        <Row popup="false" name="A3">
          <Rhs vislevel="3" flags="7">
            ...
            <Subcolumn vislevel="1">
              ...
              <Row name="reference">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="1160" window_y="397" window_width="750" window_height="750" image_left="3413" image_top="2702" image_mag="2" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="adjust">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="126" window_y="397" window_width="750" window_height="750" image_left="3322" image_top="2702" image_mag="2" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              ...
              <Row name="bp1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="178" top="924" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="3366" top="2711" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
    ...
    <Column x="862" y="0" open="true" selected="true" sform="false" next="1" name="B" caption="match vis-induced IR luminescence to vis">
      <Subcolumn vislevel="3">
        <Row popup="false" name="B1">
          <Rhs vislevel="1" flags="4">
            <Subcolumn vislevel="0"/>
        ...
        <Row popup="false" name="B2">
          <Rhs vislevel="3" flags="7">
            ...
            <Subcolumn vislevel="1">
              ...
              <Row name="reference">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="690" window_y="411" window_width="750" window_height="750" image_left="174" image_top="3761" image_mag="4" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="adjust">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="1212" window_y="397" window_width="750" window_height="750" image_left="183" image_top="3769" image_mag="4" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
              <Row name="ap1">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="166" top="3775" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="bp2">
                <Rhs vislevel="1" flags="1">
                  <iArrow left="1880" top="342" width="0" height="0">
                    <iRegiongroup/>
                  </iArrow>
                  <Subcolumn vislevel="0"/>
                  <iText/>
              ...
              <Row name="test_alignment">
                <Rhs vislevel="1" flags="1">
                  <iImage window_x="1" window_y="52" window_width="750" window_height="750" image_left="200" image_top="3736" image_mag="4" show_status="true" show_paintbox="false" show_convert="false" show_rulers="false" scale="1" offset="0" falsecolour="false" type="true"/>
                  <Subcolumn vislevel="0"/>
                  <iText/>
                </Rhs>
              </Row>
  ...
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="0" lpane_open="false" rpane_position="400" rpane_open="false" local_defs="// private definitions for this workspace&#10;" name="specden" caption="spectral density" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
    <Column x="421" y="0" open="true" selected="false" sform="false" next="35" name="O" caption="match brightness of IR to vis">
      <Subcolumn vislevel="3">
    ...
    <Column x="1137" y="0" open="true" selected="false" sform="false" next="1" name="A" caption="match brightness of UV to vis">
      <Subcolumn vislevel="3">
        <Row popup="false" name="A7">
          <Rhs vislevel="3" flags="7">
            <Matrix/>
    ...
    <Column x="710" y="0" open="true" selected="true" sform="false" next="1" name="E" caption="get the spectralon from the UV reflectance image">
      <Subcolumn vislevel="3">
  ...
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="0" lpane_open="false" rpane_position="400" rpane_open="false" local_defs="// private definitions for this workspace&#10;" name="uvlstray" caption="remove stray light from UV-induced visible luminescence image" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
    <Column x="784" y="0" open="true" selected="true" sform="false" next="48" name="Q" caption="remove residual visible light">
      <Subcolumn vislevel="3">
  ...
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="0" lpane_open="false" rpane_position="400" rpane_open="false" local_defs="// private definitions for this workspace&#10;" name="vilstray" caption="remove stray light from vis-induced IR luminescence image" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
    <Column x="784" y="0" open="true" selected="true" sform="false" next="48" name="Q" caption="remove residual visible light">
      <Subcolumn vislevel="3">
        <Row popup="false" name="Q1">
          <Rhs vislevel="2" flags="6">
  ...
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="0" lpane_open="false" rpane_position="400" rpane_open="false" local_defs="// private definitions for this workspace&#10;" name="vilkm" caption="Kubelka-Munk processing of visible-induced IR luminescence image" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
    <Column x="609" y="0" open="true" selected="true" sform="false" next="6" name="I" caption="result">
      <Subcolumn vislevel="3">
        <Row popup="false" name="I1">
          <Rhs vislevel="2" flags="6">
            <Subcolumn vislevel="1"/>
  ...
  <Workspace window_x="1" window_y="28" window_width="958" window_height="1171" view="WORKSPACE_MODE_REGULAR" scale="1" offset="0" lpane_position="0" lpane_open="false" rpane_position="100" rpane_open="false" local_defs="// private definitions for this workspace&#10;&#10;export x&#10;&#9;= oo_unary_function export_op x, is_class x&#10;&#9;= icc_export input.depth &quot;$VIPSHOME/share/$PACKAGE/data/sRGB.icm&quot; Render_intent.RELATIVE (lab x), is_image x&#10;&#9;= error (_ &quot;bad arguments to &quot; ++ &quot;export&quot;)&#10;{&#10;&#9;export_op = Operator $export export Operator_type.COMPOUND false;&#10;&#10;&#9;lab x = colour_transform_to Image_type.LAB (get_image x);&#10;}&#10;&#10;" name="results" caption="all the finished images" filename="$HOME/GIT/bm-workspaces/bm-workspaces2.ws">
    ...
    <Column x="1139" y="0" open="true" selected="true" sform="false" next="13" name="C" caption="luminescence images">
      <Subcolumn vislevel="3">
